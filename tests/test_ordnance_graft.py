...
def test_skyhook_blocked_after_drone_dies():
    ...
    gate fails. The slot itself is replaced by Jaunt; see the Jaunt swap tests."""
    g = fresh_game()
    ts = free_tiles(g, 40)
    graft = place(g, UnitType.ORDNANCE_GRAFT, 1, *ts[0])
    sky = next(s for s in graft.active_skills if s.name == "Skyhook")
...
def test_jaunt_swaps_in_when_drone_dies_and_back_on_regen():
    """Drone death replaces Skyhook with Jaunt in the same slot; regenerating the drone
    ...
    g.current_player = 1
    g._process_ordnance_graft_upkeep()  # spawn drone
    check("jaunt_starts_skyhook", _leap_slot(graft).name == "Skyhook",
          f"slot={_leap_slot(graft).name} (want Skyhook with drone alive)")
    _leap_slot(graft).current_cooldown = 3
    drone = graft.drone
    drone.hp = 0
    g.handle_unit_death(drone, None, cause="test", ui=None)
    check("jaunt_swaps_in_on_drone_death", _leap_slot(graft).name == "Jaunt",
          f"slot={_leap_slot(graft).name} (want Jaunt after drone dies)")
    check("jaunt_carries_cooldown_in", _leap_slot(graft).current_cooldown == 3,
...
def _make_jaunt_graft(g, gy, gx):
    """Place a graft, give it a drone, then kill the drone so the "S" slot holds Jaunt."""
...
def test_jaunt_requires_anchor_los_and_range():
    """Jaunt can only grab a real anchor (unit / furniture / solid terrain) that is in LOS
    and within its shorter range; an empty tile, an out-of-range anchor, or an anchor behind
    a wall are all rejected."""
    from boneglaive.game.map import TerrainType
    g = fresh_game()
    g.unit_grid = {}
    graft = _make_jaunt_graft(g, 5, 5)
    j = _leap_slot(graft)
    check("jaunt_is_active", j is not None and j.name == "Jaunt", "slot holds Jaunt")
    # Empty passable tile in range -> not an anchor.
    check("jaunt_rejects_empty_tile", j.can_use(graft, (5, 8), g) is False,
          "empty passable tile is not a grabbable anchor")
    # Enemy anchor in LOS at range 3 -> OK.
    enemy = place(g, UnitType.GLAIVEMAN, 2, 5, 8)
    check("jaunt_accepts_enemy_anchor", j.can_use(graft, (5, 8), g) is True,
          "enemy in LOS at dist 3 is grabbable")
    # Out of range (dist 4).
    place(g, UnitType.GLAIVEMAN, 2, 5, 9)
    check("jaunt_rejects_out_of_range", j.can_use(graft, (5, 9), g) is False,
          "anchor at dist 4 > range 3 rejected")
    # Wall as anchor (solid terrain) at dist 2.
    g.map.set_terrain_at(5, 7, TerrainType.LIMESTONE)
    check("jaunt_accepts_wall_anchor", j.can_use(graft, (5, 7), g) is True,
          "solid terrain is grabbable")
    # LOS blocked: a wall between him and a far enemy.
    g2 = fresh_game()
    g2.unit_grid = {}
    gr = _make_jaunt_graft(g2, 5, 5)
    jj = _leap_slot(gr)
    g2.map.set_terrain_at(5, 6, TerrainType.LIMESTONE)   # wall right in front
    place(g2, UnitType.GLAIVEMAN, 2, 5, 8)               # enemy behind it
    check("jaunt_rejects_blocked_los", jj.can_use(gr, (5, 8), g2) is False,
          "cannot grapple an enemy through a wall")


def test_jaunt_pulls_to_stop_adjacent_and_slams():
    """Jaunt reels him in a straight line to stop one tile short of the anchor, then runs
    ...
    e_hp0 = enemy.hp
    check("jaunt_can_use_ok", j.can_use(graft, (5, 8), g) is True, "setup")
    j.use(graft, (5, 8), g)
    ...
    check("jaunt_executes", ok is True, f"returned={ok}")
    check("jaunt_stops_adjacent", (graft.y, graft.x) == (5, 7),
          f"landed={(graft.y, graft.x)} (want (5,7), one short of the anchor)")
    check("jaunt_slam_strikes", enemy.hp == e_hp0 - expected_strike,
          f"enemy {e_hp0}->{enemy.hp} (want -{expected_strike} after DEF)")
    check("jaunt_slam_plants", len(enemy.bombs) == 1,
          f"bombs={len(enemy.bombs)} (want 1)")


def test_jaunt_aborts_and_refunds_with_no_anchor():
    """If the target isn't a valid anchor at execute (e.g. it moved/died), Jaunt aborts
    ...
    ok = j.execute(graft, (5, 8), g, ui=None)
    check("jaunt_aborts_no_anchor", ok is False and (graft.y, graft.x) == before,
          f"returned={ok} moved={(graft.y, graft.x) != before} (want abort, no move)")
    check("jaunt_refunds_no_anchor", j.current_cooldown == 0,
          f"cd={j.current_cooldown} (want refunded to 0)")


def test_jaunt_reels_from_queued_move_destination_not_pre_move_tile():
    """Move + Jaunt: he must reel from where he MOVED to, not his pre-move tile.
    ...
    check("jaunt_movedest_can_use", j.can_use(graft, (5, 8), g) is True,
          "anchor reachable from the queued destination")

    j.use(graft, (5, 8), g)
    check("jaunt_movedest_captures_origin", graft.jaunt_launch_from == (5, 5),
          f"launch_from={graft.jaunt_launch_from} (want the move destination (5,5))")
    check("jaunt_movedest_clears_move", graft.move_target is None,
          "the leap replaces the walk -> move_target cleared")

    expected_strike = max(1, STRIKE_DAMAGE - enemy.get_effective_stats()['defense'])
    ok = j.execute(graft, (5, 8), g, ui=None)
    check("jaunt_movedest_executes", ok is True, f"returned={ok}")
    check("jaunt_movedest_lands_from_destination", (graft.y, graft.x) == (5, 7),
          f"landed={(graft.y, graft.x)} (want (5,7) reeled from the destination, NOT blocked at (6,6))")
    check("jaunt_movedest_slam_strikes", enemy.hp == e_hp0 - expected_strike,
          f"enemy {e_hp0}->{enemy.hp} (want -{expected_strike}; proves he reached the anchor)")
    check("jaunt_movedest_consumes_launch", graft.jaunt_launch_from is None,
    ...
    check("jaunt_movedest_anim_from_set", graft.skill_walkin_from == (5, 5),
          f"skill_walkin_from={graft.skill_walkin_from} (want the move destination (5,5))")


def test_jaunt_walkin_unset_without_queued_move():
    """An in-place Jaunt (no queued move) leaves skill_walkin_from None, so the animation
    ...
    check("jaunt_no_move_anim_from_none", graft.skill_walkin_from is None,
...
def main():
    ...
    test_jaunt_aborts_and_refunds_with_no_anchor()
    test_jaunt_reels_from_queued_move_destination_not_pre_move_tile()
